...
pub struct Season7PowerShiftLeaderboardEntry {
    pub rank: u32,
    pub points: u64,
}

#[derive(Deserialize)]
struct Season7PowerShiftRow {
    #[serde(rename = "1")]
    rank: u32,
...
impl From<Season7PowerShiftRow> for PlayerLeaderboardData {
    fn from(r: Season7PowerShiftRow) -> Self {
        ...
        let entry = Season7PowerShiftLeaderboardEntry {
            rank: r.rank,
            points: r.points,
        };

        let leaderboard_data = LeaderboardData {
            season_7_power_shift_leaderboard_entry: Some(entry),
            ..Default::default()
        };
...
pub fn parse(row: &Value) -> Option<PlayerLeaderboardData> {
    ...
    let row: Season7PowerShiftRow = serde_json::from_value(Value::Object(obj.clone())).ok()?;
    Some(row.into())
}