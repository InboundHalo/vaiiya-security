...
pub struct Season7WorldTourLeaderboardEntry {
    pub rank: u32,
    pub points: u64,
}

#[derive(Deserialize)]
struct Season7WorldTourRow {
    #[serde(rename = "1")]
    rank: u32,
...
impl From<Season7WorldTourRow> for PlayerLeaderboardData {
    fn from(r: Season7WorldTourRow) -> Self {
        ...
        let entry = Season7WorldTourLeaderboardEntry {
            rank: r.rank,
            points: r.points,
        };

        let leaderboard_data = LeaderboardData {
            season_7_world_tour_leaderboard_entry: Some(entry),
            ..Default::default()
        };
...
pub fn parse(row: &Value) -> Option<PlayerLeaderboardData> {
    ...
    let row: Season7WorldTourRow = serde_json::from_value(Value::Object(obj.clone())).ok()?;
    Some(row.into())
}