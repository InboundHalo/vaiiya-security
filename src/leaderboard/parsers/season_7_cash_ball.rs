...
pub struct Season7CashBallLeaderboardEntry {
    pub rank: u32,
    pub points: u64,
}

#[derive(Deserialize)]
struct Season7CashBallRow {
    #[serde(rename = "1")]
    rank: u32,
...
impl From<Season7CashBallRow> for PlayerLeaderboardData {
    fn from(r: Season7CashBallRow) -> Self {
        ...
        let entry = Season7CashBallLeaderboardEntry {
            rank: r.rank,
            points: r.points,
        };

        let leaderboard_data = LeaderboardData {
            season_7_cash_ball_leaderboard_entry: Some(entry),
            ..Default::default()
        };
...
pub fn parse(row: &Value) -> Option<PlayerLeaderboardData> {
    ...
    let row: Season7CashBallRow = serde_json::from_value(Value::Object(obj.clone())).ok()?;
    Some(row.into())
}